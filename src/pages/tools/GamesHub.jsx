/**
 * GamesHub — tab content shown on the Dashboard under 🎮 Games
 * Renders all game cards from GameRegistry. Only shown when logged in.
 */
...
function GameCard({ game }) {
  ...
  const handleClick = () => {
    if (hasAccess) navigate(game.route);
    ...
    <div
      ...
      <div className="flex items-start justify-between">
        ...
        <div className="flex gap-2 items-center">
          ...
          {!hasAccess && (
            <span className="text-[10px] bg-yellow-700/60 text-yellow-300 px-2 py-0.5 rounded-full font-bold flex items-center gap-1">
  ...
  return (
    <div>
      ...
      <p className="text-gray-400 mb-6 text-sm">
        ...
        Each game generates a personal overlay URL for use as a Browser Source in TikTok Live Studio.
      </p>
      ...
      <div className="mt-8 bg-[#151828] border border-[#1e2240] rounded-xl p-5 text-sm text-gray-500">
        ...
        Each game has a unique overlay URL tied to your account.
        Add it as a Browser Source in TikTok Live Studio. Viewers interact via chat commands — no app download needed.
      </div>
    </div>
  );
}
